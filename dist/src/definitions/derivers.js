...
const makeDeriver = def => {
    ...
    return R.merge(def, {
        fn: ({ dataset }, inputs) => {
            ...
            return dataset.appendColumn(Column(inputs.columnName, vals, Column.detectSchema(vals)));
        },
        help: 'help text',
        tags: ["deriver"],
...
const Logarithm = makeDeriver({
    ...
    display: ({ dataset }, { base, num }) => h('div', {}, [
        `Log base ${base} of `,
        col(dataset, num)
    ])
});
const Quantile = makeDeriver({
    ...
    fn: (inputs) => {
        const sorted = R.pipe(R.map(parseFloat), R.sort((a, b) => a - b))(inputs.n);
        const frac = parseFloat(sorted.length) / parseFloat(inputs.order);
        const cutoffs = R.map(n => R.nth(Math.ceil(n * frac), sorted), R.range(0, parseInt(inputs.order)));
        return R.map(n => R.findLastIndex(m => parseFloat(n) >= m, cutoffs) + 1, inputs.n);
    },
