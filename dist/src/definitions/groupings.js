...
const Grouping = {
    ...
    fn: ({ dataset }, inputs) => {
        ...
        const groups = R.pipe(R.groupBy(record => JSON.stringify(R.map(R.nth(R.__, record), columns))), R.toPairs)(dataset.records);
        const groupingCols = R.pipe(R.nth(0), JSON.parse, mapIndexed((vals, i) => Column.autoSchema(headers[i], vals)));
        const groupedRows = R.map(agg => R.map(group => agg.fn(group[1])), // returns a list of Columns
        aggregators);
