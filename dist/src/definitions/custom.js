...
const SoccerRanking = (function () {
    ...
    return {
        ...
        fn: ({ dataset }, inputs) => {
            ...
            const outcomeReducer = R.reduce((acc, t) => ({ points: acc.points + t.points, goalDiff: acc.goalDiff + t.goalDiff }), { points: 0, goalDiff: 0 });
            const outcomes = R.pipe(toLines, R.groupBy(R.prop('team')), R.map(outcomeReducer), R.mapObjIndexed((v, k) => R.merge({ team: k }, v)), R.values, R.sortWith([R.descend(R.prop('points')), R.descend(R.prop('goalDiff'))]), R.addIndex(R.map)((t, i) => [(i + 1).toString(), t.team, t.points.toString(), t.goalDiff.toString()]))(rows);
            return Dataset.fromCSV({
                headers: ['Rank', 'Team', 'Points', 'Goal Difference'],
                records: outcomes
            });
        },
        display: ({ dataset }, inputs) => h('div', {}, [
            'Soccer Rankings'
        ]),
        valid: ({ dataset }, inputs) => R.all(slot => slot.valid(dataset, inputs[slot.id]), slots)
    };
}());
module.exports = {
    SoccerRanking
