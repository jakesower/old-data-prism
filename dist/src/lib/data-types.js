...
const NonEmptyString = withDefaults({
    ...
    test: x => x !== "",
    cast: x => x,
    testCast: x => x === "" ? either_1.Err("must not be empty") : either_1.Ok(x),
});
const _Number = withDefaults({
    name: "Number",
    test: x => !isNaN(parseFloat(x)),
    cast: parseFloat,
    testCast: x => !isNaN(parseFloat(x)) ? either_1.Ok(parseFloat(x)) : either_1.Err("must be numeric"),
});
const FiniteNumber = withDefaults({
    name: "Finite Number",
    test: finiteNum,
    cast: parseFloat,
    testCast: x => finiteNum(x) ? either_1.Ok(parseFloat(x)) : either_1.Err("must be a finite number"),
});
