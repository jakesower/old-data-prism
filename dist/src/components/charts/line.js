...
function fn(dataset, inputs, dimensions) {
    ...
    if (R.isEmpty(rowAxis) || R.isEmpty(colAxes))
        return [];
    const columns = dataset.columns;
    const rangePipe = R.pipe(R.flatten, R.map(parseFloat), R.append(0), R.apply(R.juxt([Math.min, Math.max])), R.apply(Range));
    ...
    const catGrid = R.map(R.addIndex(R.map)((y, idx) => Point(idx, y)))(grid);
    const paths = R.pipe(R.map(toPath), R.map(c => c.project(basis)), R.map(toSvgTag({})))(catGrid);
    const yZero = toSvgTag({ class: { zero: true } }, Shape.Line(Point(0, 0), Point(xRange.max, 0)).project(basis));
    return paddedSvg(dimensions, 10, { class: { 'scatter-plot': true } }, R.flatten([
        paths,
        [yZero]
    ]));
}
